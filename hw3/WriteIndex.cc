...
static int WriteHashTable(FILE *f, IndexFileOffset_t offset, HashTable *ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int bucket_record_bytes = WriteHTBucketRecord(f, record_pos, num_elts,
      bucket_pos);
    if (bucket_record_bytes != sizeof(BucketRecord)) {
      return kFailedWrite;
    }
    record_pos += bucket_record_bytes;
...
static int WriteHTBucket(FILE *f, IndexFileOffset_t offset, LinkedList *li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    if (fwrite(&element_record, sizeof(ElementPositionRecord), 1, f) != 1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
    ...
    LLPayload_t pay;
    LLIterator_Get(it, &pay);
    int element_size = fn(f, element_pos, (HTKeyValue_t*) pay);
    if (element_size < 0) {
      LLIterator_Free(it);
      return (kFailedWrite);
    }
    ...
    element_pos += element_size;  // you may need to change this logic
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  if (fwrite(kv->value, sizeof(char), file_name_bytes, f) != file_name_bytes) {
    return kFailedWrite;
  }
  // STEP 11.
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    DocPositionOffset_t pos = static_cast<DocPositionOffset_t>(
      reinterpret_cast<uintptr_t>(pay));
  ...
  int total_bytes = sizeof(DocIDElementHeader) + (num_positions *
    sizeof(DocIDElementPosition));
  return total_bytes;  // you may need to change this return value
}
